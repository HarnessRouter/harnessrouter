...
from server import (Auth, BACKENDS, CHECKPOINT_EXCLUDE, _KIMI_TOOLS,  # noqa: E402
                    _kimi_mcp_unavailable, _HERMES_RELAY, _drop_top_level_field,
...
def test_the_two_fields_the_cli_invents_never_reach_the_provider():
    """Captured at a stub on 2.0.0: max_tokens 131072 on every request and reasoning_effort "high"
    for a claude-named model. Through the product they failed gpt, claude and gemini ids on the
    output limit, five newer Claude ids on thinking.type.enabled, and llama-3.3-70b on its window.
    The route is born dropping both, so the provider's defaults apply as on every other base."""
    _, _, env = _argv()
