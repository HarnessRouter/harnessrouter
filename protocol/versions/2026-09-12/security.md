...
## 9. Plugins are third-party code

A plugin ([Plugins](plugins.md)) is instructions and executables written by someone other than the
operator installing it. Installing one extends the harness's trust to its author.

- A stdio MCP server declared by a plugin MUST run inside the sandbox that runs the agent, with the
  agent's privileges and no more. It MUST NOT run on the server host, and MUST NOT be launched
  through a shell.
- A server MUST expand only `${PLUGIN_ROOT}` and `${PLUGIN_DATA}`, only in `args`, `env` and `cwd`,
  and MUST NOT perform any other placeholder or environment-variable expansion. A plugin that could
  read the sandbox's environment into its own process arguments would read whatever credentials the
  sandbox holds ([§1](#1-credentials)).
- A `command` or `cwd` that resolves outside the plugin root is an invalid entry, never a permitted
  one ([Plugins §6](plugins.md#6-running-with-plugins)).
- Plugins are installed by whoever manages the harness (class Full), never per request. A
  per-request plugin would be a per-request MCP server, which [Tasks §1.4](tasks.md#14-reserved-fields-tools-and-include)
  already declines for the same reason: narrowing is safe, widening is escalation.
- A skill inside a plugin is prompt content and is subject to [§4](#4-prompt-injection-is-in-scope-for-the-client).
  A client SHOULD show the manifest's `author`, `homepage` and `repository` before installing, and
  SHOULD show `skipped` after, so that what did not load is not mistaken for what did.
- A server MUST omit `auth` and `headers` from an exported package
  ([Plugins §5](plugins.md#5-exporting-a-harness-as-a-plugin)). An export is meant to leave.
