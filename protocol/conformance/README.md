...
That run predates the `T-` additions and the `R-` series, so it is a 52-check result of the
62-check suite. A remeasured run replaces this block via the `conformance-remeasure` workflow.
