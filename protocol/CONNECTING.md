...
A UHP [client](versions/2026-09-12/architecture.md#1-roles) discovers the harnesses a server
...
is the procedure; the [Architecture chapter](versions/2026-09-12/architecture.md#1-roles) is
the definition.
...
harness are in the [Architecture chapter](versions/2026-09-12/architecture.md#1-roles) — this guide is
...
[schema chapter](versions/2026-09-12/schema.md) — generate a typed client for your language from it and
...
1. **Discover the catalog.** Ask the server which harnesses it runs and which models each
   ...
   server-assigned id. [Harnesses chapter](versions/2026-09-12/harnesses.md).
2. **Submit a task.** One unit of work: input in, result out. Name the configured harness and
   send the request. [Tasks chapter](versions/2026-09-12/tasks.md).
3. **Consume the stream.** Read the event stream to follow progress while the task runs, and
   ...
   events. [Streaming chapter](versions/2026-09-12/streaming.md).
4. **Continue in a session.** To pick up where a task left off, carry the session forward
   instead of starting cold. [Sessions chapter](versions/2026-09-12/sessions.md).
5. **Take the artifacts.** Download the files the run produced. [Files chapter](versions/2026-09-12/files.md).
6. **Cancel and handle errors.** Stop work cleanly, and read failures from the documented
   error taxonomy rather than guessing from status codes. [Errors chapter](versions/2026-09-12/errors.md).
...
Generate the exact request and response bodies from the [schema](versions/2026-09-12/schema.md); the
